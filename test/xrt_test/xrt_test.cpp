...
usage(const std::string& prog)
{
  ...
  std::cout << "\t" << "-x" << ": specify xclbin and elf to use (nop and stress hwctx only)\n";
...
/* Execute c_rounds batches with up to o_cmds outstanding. When r_cmds==1 each
   ...
   runs must have size o_cmds * max(r_cmds, 1). Uses global r_cmds, o_cmds,
   c_rounds, timeout_ms. */
static void
execute_batches(xrt::hw_context& hwctx, std::vector<xrt::run>& runs)
{
  if (r_cmds == 1) {
    /* runs.size() == o_cmds; submit up to c_rounds total, at most o_cmds outstanding */
    int submitted = 0;
    int completed = 0;
    while (submitted < static_cast<int>(runs.size()) && submitted < static_cast<int>(c_rounds)) {
    ...
    while (completed < submitted) {
      ...
      if (submitted < static_cast<int>(c_rounds)) {
        runs[i].start();
        submitted++;
      }
    }
  ...
  while (submitted < static_cast<int>(runlists.size()) && submitted < static_cast<int>(c_rounds)) {
...
TEST_xrt_umq_single_col_resnet50_all_layer(int device_index, arg_type& arg)
{
  ...
  for (int i = 0; i < static_cast<int>(OFM_BYTE_SIZE / sizeof(uint32_t)); i++) {
    uint32_t gld;
    ofm_ifs >> std::hex >> gld;
    if (gld != ofm[i]) { std::cout << "[ERROR]: No." << i << std::hex << "   golden = 0x" << gld << ", ofm = 0x" << ofm[i] << std::endl; err++; }
  }
  if (err) throw std::runtime_error("result mis-match");
...
TEST_xrt_umq_runlist(int device_index, arg_type& arg) {
  unsigned original_r_cmds = r_cmds;  
  r_cmds = 24;
  ...
  xrt::hw_context hwctx{device, elf};
  xrt::kernel kernel = xrt::ext::kernel{hwctx, "DPU:dpu"};
  unsigned n = o_cmds * r_cmds;
  std::vector<xrt::run> runs;
  for (unsigned i = 0; i < n; i++)
    runs.emplace_back(kernel);
  execute_batches(hwctx, runs);
  r_cmds = original_r_cmds;
}
    ...
    while ((option = getopt(argc, argv, ":c:o:r:m:x:i:t:e:v:w:lh")) != -1) {
	  ...
	  break;
        ...
        case 'r': {
          val = std::stoi(optarg);
          if (val > 24) {
            std::cout << "r_cmds max is 24" << std::endl;
            return 1;
          }
