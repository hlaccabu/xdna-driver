    ...
    for (auto& cmd : cmdlist_bos) {
      ...
      if (cmd_pkt->opcode == ERT_CMD_CHAIN) {
        ...
        for (size_t i = start_idx; i < end_idx; i++) {
          ...
          io_test_bo_set_base::restore_cmd_header(subcmd_bo->get(), subcmd_pkt);
        }
      }

      io_test_bo_set_base::restore_cmd_header(cmd_hdl, cmd_pkt);
      io_test_bo_set_base::cache_cmd_header(cmd_hdl, cmd_pkt);
  ...
  int cmds_per_list, const char *xclbin)
{
  ...
  if (cmds_per_list == 1) {
    ...
    for (auto& boset : bo_set) {
      ...
      auto subcmd_pkt = reinterpret_cast<ert_start_kernel_cmd *>(subcmd_bo->map());
      io_test_bo_set_base::cache_cmd_header(subcmd_bo->get(), subcmd_pkt);
      ...
      if ((tmp_cmd_bos.size() % cmds_per_list) == 0) {
        ...
        io_test_bo_set_base::cache_cmd_header(cbo->get(), cmdpkt);
        tmp_cmd_bos.clear();
