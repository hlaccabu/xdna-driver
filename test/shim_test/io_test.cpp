...
TEST_io_runlist_bad_cmd(device::id_type id, std::shared_ptr<device>& sdev, arg_type& arg)
{
  ...
  const auto& good_info = get_binary_info(dev, good_tag, nullptr);
  flow_type good_flow = good_info.flow;

  // Two good ones
  auto good_bo_set1 = create_bo_set_for_device(dev, false, good_tag);
  auto good_bo_set2 = create_bo_set_for_device(dev, false, good_tag);
  // A timeout one
