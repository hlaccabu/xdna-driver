...
uint32_t io_test_bo_set_base::s_cached_subcmd_header;
...
cache_cmd_header(const buffer_handle *cmd_hdl, const ert_start_kernel_cmd *cmd)
{
  if (cmd->opcode != ERT_CMD_CHAIN)
    s_cached_subcmd_header = cmd->header;
}
...
{
  if (cmd->opcode != ERT_CMD_CHAIN && s_cached_subcmd_header != 0)
    cmd->header = s_cached_subcmd_header;
}
