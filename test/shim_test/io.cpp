...
create_bo_set_for_device(device* dev, bool use_ubuf, const char* tag)
{
  ...
  const auto& info = get_binary_info(dev, effective_tag, nullptr);
  ...
  , m_expect_cmd_status(0)
{
  ...
  if (info.flow == FULL_ELF) {
    ...
    m_expect_cmd_status = info.extra.count("exp_status")
      ? static_cast<uint32_t>(std::stoul(info.extra.at("exp_status"), nullptr, 0))
      : static_cast<uint32_t>(ERT_CMD_STATE_TIMEOUT);
...
verify_result()
{
  ...
  if (cdata->npu_gen == NPU_GEN_AIE4) {
    if (cdata->version != ERT_CTX_HEALTH_DATA_V1) {
