...
{
	struct amdxdna_hwctx_param_config_scheduling *scheduling =
	       (struct amdxdna_hwctx_param_config_scheduling *)buf;
	DECLARE_AIE4_MSG(aie4_msg_configure_hw_context, AIE4_MSG_OP_CONFIGURE_HW_CONTEXT);
	struct amdxdna_dev *xdna = ctx->client->xdna;
	struct amdxdna_dev_hdl *ndev = xdna->dev_handle;
	int ret;
...
{
	struct amdxdna_hwctx_param_config_dpm *dpm = (struct amdxdna_hwctx_param_config_dpm *)buf;
	DECLARE_AIE4_MSG(aie4_msg_configure_hw_context, AIE4_MSG_OP_CONFIGURE_HW_CONTEXT);
	struct amdxdna_dev *xdna = ctx->client->xdna;
	struct amdxdna_dev_hdl *ndev = xdna->dev_handle;
	int ret;
